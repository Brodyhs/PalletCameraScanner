...
def make_pass(payload, sym, ppm, speed_pxps, blur_px, fps, noise_sigma=4.0, bg=120, seed=0):
    ...
    # ~40 frames ≈ a 0.7 s dwell at 55 fps (a realistic single pass), so every
    # speed is tested with the same number of decode chances and the sweep is fast.
    n = 40
    frames = []
    for i in range(n):
        x = int(i * pxpf)
...
def sweep(sym, ppm, module_mm, fps, payload):
    ...
    print("cell = # frames decoded in the pass (higher = more robust); '.' = MISS\n")
    print("exp \\ mph " + "".join(f"{m:>7}" for m in mphs))
    for e in exps:
        cells = []
        for m in mphs:
            ...
            cells.append(r["decode_frames"])
        print(f"{e:>4}ms   " + "".join((f"{v:>7}" if v else "      .") for v in cells))
